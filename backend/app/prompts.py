...
DEFENDER_PROMPT = ChatPromptTemplate.from_messages([
    ("system",
     ...
     "  Special attention: (a) locations where the individual lives or works are strong quasi-identifiers — "
     "always generalise to a region or country, never leave a specific city or village; "
     "(b) named political movements, armed groups, or organisations the individual belongs to are "
     "quasi-identifiers — replace with a generic description (e.g. 'an illegal organisation'); "
     "(c) never leave a specific date verbatim if it is listed as an attribute to hide — at minimum "
     "generalise to a month/year or a relative reference ('in the late 1990s').\n"
